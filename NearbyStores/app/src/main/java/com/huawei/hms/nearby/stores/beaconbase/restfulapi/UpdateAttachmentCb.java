 ...
 * @since 2020-01-13
 */
public class UpdateAttachmentCb implements Callback<UpdateAttachmentRes> {
    private static final int FAIL_UNKNOW_UPDATE_ATTACH = -1;

    private static final int FAIL_TIMEOUT_UPDATE_ATTACH = -5;

    private static final String TAG = "UpdateAttachmentCb";

    /**
     * Semaphone for wait cloud request complete
     */
    private Semaphore semaphoreUpdateAttach;
     ...
     * Constructor
     */
    public UpdateAttachmentCb() {
        semaphoreUpdateAttach = new Semaphore(0);
        result = FAIL_TIMEOUT_UPDATE_ATTACH;
    }
    ...
    /**
     * Wait update attachment request complete, and get the result code
     *
     * @return update attachment request result:
    ...
    public long waitComplete() {
        try {
            if (semaphoreUpdateAttach.tryAcquire(3, TimeUnit.SECONDS)) {
                return result;
            }
            return FAIL_UNKNOW_UPDATE_ATTACH;
    ...
    public void onFailure(Call<UpdateAttachmentRes> call, Throwable throwable) {
        ...
        result = FAIL_UNKNOW_UPDATE_ATTACH;
        semaphoreUpdateAttach.release();
    }
}
